...
\ifx\putwordSee\undefined       \gdef\putwordSee{See}\fi
\ifx\putwordShortTOC\undefined  \gdef\putwordShortTOC{Short contexts}\fi
\ifx\putwordTOC\undefined       \gdef\putwordTOC{Table of contexts}\fi
%
...
% page or the contexts), so we use \firstmark there -- this gets us
% the mark with the chapter defs, unless the user sneaks in, e.g.,
% @setcolor (or @url, or @link, etc.) between @contexts and the very
% first @chapter.
...
%
\chardef\PAGE = 255
\output = {\onepageout{\pagecontexts\PAGE}}
...
{\catcode`\@ =11
\gdef\pagecontexts#1{\ifvoid\topins\else\unvbox\topins\fi
...
% Fonts for short table of contexts.
...
% Do an implicit @contexts or @shortcontexts after @end titlepage if the
% user says @setcontextsaftertitlepage or @setshortcontextsaftertitlepage.
%
\newif\ifsetcontextsaftertitlepage
 \let\setcontextsaftertitlepage = \setcontextsaftertitlepagetrue
\newif\ifsetshortcontextsaftertitlepage
 \let\setshortcontextsaftertitlepage = \setshortcontextsaftertitlepagetrue
...
\def\Etitlepage{%
  ...
  \ifsetshortcontextsaftertitlepage
    \shortcontexts
    \contexts
    \global\let\shortcontexts = \relax
    \global\let\contexts = \relax
  \fi
  %
  \ifsetcontextsaftertitlepage
    \contexts
    \global\let\contexts = \relax
    \global\let\shortcontexts = \relax
  \fi
...
\global\let\contextsalignmacro = \chapoddpage
}
\let\contextsalignmacro = \chappager
...
\global\oddheadline={\line{\thischapterheading\hfil\folio}}
\global\let\contextsalignmacro = \chappager
}
...
\def\doitemize#1{%
  ...
  \def\itemcontexts{#1}%
  \setbox0 = \hbox{\itemcontexts}%
  %
  % @itemize with no arg is equivalent to @itemize @bullet.
  \ifx\itemcontexts\empty\def\itemcontexts{\bullet}\fi
  %
...
\def\itemizeitem{%
  ...
  {%
   ...
   \noindent
   \hbox to 0pt{\hss \itemcontexts \kern\itemmargin}%
   %
   \ifinner\else
...
% contexts of each column will wrap at width given in preamble.  Width
...
% and table of contexts entries.  The paragraph is indented by \leftskip.
%
\def\entry{%
  \begingroup
    %
...
\def\begindoublecolumns{\begingroup % ended by \enddoublecolumns
  ...
  \output = {%
    ...
    \ifvoid\partialpage \else
      \onepageout{\pagecontexts\partialpage}%
    \fi
    %
...
%
% Re-output the contexts of the output page -- any previous material,
...
\def\enddoublecolumns{%
  ...
  \output = {%
    ...
    \global\output = {\onepageout{\pagecontexts\PAGE}}%
  }%
  \eject
...
\def\partzzz#1{%
  ...
  \begingroup
    ...
    % and \thissection, as is done in \startcontexts.
...
\def\unnumberedzzz#1{%
  ...
  % simply yielding the contexts of <toks register>.  (We also do this for
  % the toc entries.)
  \toks0 = {#1}%
  \message{(\the\toks0)}%
  %
...
\def\chapmacro#1#2#3{%
  ...
  {%
    ...
    \ifx\temptype\Ynothingkeyword
      ...
      \def\toctype{unnchap}%
    \else\ifx\temptype\Yomitfromtockeyword
      \setbox0 = \hbox{}% contexts like unnumbered, but no toc entry
      \def\toctype{omit}%
    \else\ifx\temptype\Yappendixkeyword
...
% any other fixed time) so that @contexts can be anywhere in the document.
% But if #1 is `omit', then we don't do anything.  This is used for the
% table of contexts chapter openings themselves.
%
...
\def\writetocentry#1#2#3{%
  ...
  % writing pdf.  These are used in the table of contexts.  We can't
...
\newskip\contextsrightmargin \contextsrightmargin=1in
\newcount\savepageno
...
%
\def\startcontexts#1{%
  % If @setchapternewpage on, and @headings double, the contexts should
  % start on an odd page, unlike chapters.  Thus, we maintain
  % \contextsalignmacro in parallel with \pagealignmacro.
  % From: Torbjorn Granlund <tege@matematik.su.se>
  \contextsalignmacro
  \immediate\closeout\tocfile
  %
  % Don't need to put `contexts' or `Short contexts' in the headline.
  % It is abundantly clear what they are.
  \chapmacro{#1}{Yomitfromtoc}{}%
  %
  \savepageno = \pageno
  \begingroup                  % Set up to handle contexts files properly.
    \raggedbottom              % Worry more about breakpoints than the bottom.
    \entryrightmargin=\contextsrightmargin % Don't use the full line length.
    %
...
%
\def\contexts{%
  \startcontexts{\putwordTOC}%
    \openin 1 \tocreadfilename\space
    \ifeof 1 \else
      \readtocfile
    \fi
    \vfill \eject
    \contextsalignmacro % in case @setchapternewpage odd is in effect
    \ifeof 1 \else
      \pdfmakeoutlines
    \fi
    \closein 1
...
\let\shortcontexts = \summarycontexts

% Typeset the label for a chapter or appendix for the short contexts.
...
% These macros generate individual entries in the table of contexts.
% The first argument is the chapter or section name.
% The last argument is the page number.
% The arguments in between are the chapter number, section number, ...

% Parts, in the main contexts.  Replace the part number, which doesn't
...
% Appendices, in the main contexts.
...
\def\scanexp#1{%
  \bgroup
  % Undo catcode changes of \startcontexts and \printindex
...
% be able to disable indexing, cross-referencing, and contexts, for test runs.
...
% @listoffloats FLOATTYPE - print a list of floats like a table of contexts.
%
